...
/**
 * REST controller for managing {@link com.surabhi.naturenotes.domain.Tag}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class TagResource {
    ...
    public ResponseEntity<Tag> partialUpdateTag(@PathVariable(value = "id", required = false) final Long id, @RequestBody Tag tag)
        ...
        Optional<Tag> result = tagRepository
            .findById(tag.getId())
            .map(existingTag -> {
                if (tag.getCamping() != null) {
                    existingTag.setCamping(tag.getCamping());
                }
                if (tag.getHike() != null) {
                    existingTag.setHike(tag.getHike());
                }
                if (tag.getRoadTrip() != null) {
                    existingTag.setRoadTrip(tag.getRoadTrip());
                }
                if (tag.getInternational() != null) {
                    existingTag.setInternational(tag.getInternational());
                }
                if (tag.getSoloTravel() != null) {
                    existingTag.setSoloTravel(tag.getSoloTravel());
                }
